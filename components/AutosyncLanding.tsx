  ...
  const initAutosync = useCallback(() => {
    try {
      ...
      window.autosyncInstance = new window.Autosync({
        ...
        disableCheckout: true, // Disable Autosync's checkout page
        homeStyle: null,
        ...
        onEvent: function({ event, data }: { event: string, data: any }) {
          ...
          // Intercept ALL purchase/quote events and route to our checkout flow
          // This prevents Autosync's submit order page from showing
          if (
            event === 'submitQuote' || 
            event === 'buy' || 
            event === 'buyClick' || 
            ...
            console.log('[Autosync] Purchase/Quote event detected - opening our checkout flow directly');
            // Prevent default Autosync behavior
            if (event === 'submitOrder' || event === 'checkout') {
              console.log('[Autosync] Preventing Autosync submit order page');
            }
            ...
            // Open our checkout flow directly
            handleBuyClick();
            
            // Return false to prevent Autosync's default behavior
            return false;
          }
          
          // Prevent Autosync's cart from showing
          if (event === 'cartUpdate' || event === 'showCart' || event === 'cartOpen') {
